...
public class GroupNetworkManager : NetworkManager
{
    ...
    private void SpawnAvatars()
    {
        ...
        {
            Debug.LogError("No start positions found! Unable to spawn player avatars");
            return;
        }

        avatars = new GameObject[(int)AvatarType.Count];

        var startPosition = GetStartPosition();
        var magicCharacterInstance = Instantiate(magicCharacterPrefab, startPosition.position, startPosition.rotation);
        NetworkServer.Spawn(magicCharacterInstance);
        avatars[(int)AvatarType.Magic] = magicCharacterInstance;
        
        startPosition = GetStartPosition();
        var meleeCharacterInstance = Instantiate(meleeCharacterPrefab, startPosition.position, startPosition.rotation);
        NetworkServer.Spawn(meleeCharacterInstance);
        avatars[(int)AvatarType.Melee] = meleeCharacterInstance;
        
        startPosition = GetStartPosition();
        var rangedCharacterInstance = Instantiate(rangedCharacterPrefab, startPosition.position, startPosition.rotation);
        NetworkServer.Spawn(rangedCharacterInstance);
